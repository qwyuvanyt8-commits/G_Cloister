...
import { disconnectUserSockets, emitToRoom } from "./socket.js";
...
adminRouter.post("/users/:userId/rooms/:roomId/kick", requireAdmin, async (req, res) => {
  ...
  try {
    await db.run(
      "UPDATE room_members SET kicked = 1 WHERE room_id = ? AND user_id = ?",
      [roomId.toLowerCase(), userId]
    );
    emitToRoom(roomId.toLowerCase(), "member:kicked", { userId });
    res.json({ ok: true, userId, roomId, kicked: true });
  } catch (err) {
...
adminRouter.post("/users/:userId/rooms/:roomId/unkick", requireAdmin, async (req, res) => {
  ...
  try {
    ...
    res.json({ ok: true, userId, roomId, kicked: false });
  } catch (err) {
